...
Public Class HeadApproval1_RequestTrackerApprover
        ...
        Try
            ...
            Dim cmd As New MySqlCommand("SELECT *
                                     FROM
                                         table_osrequest
                                     WHERE
                                         department IN ('ITSO', 'FSO', 'lab', 'Marketing', 'SDAO', 'LRC', 'FMO', 'Admission') AND status1 = 'Approved'
                                     ORDER BY
                                         CASE WHEN status1 = 'Approved' THEN 0 ELSE 1 END", con)
    ...
    Private Sub MoveRowsToHistory()
        Try
            ...
            Dim cmd As New MySqlCommand("INSERT INTO table_requesthistory SELECT * FROM table_osrequest WHERE status2 IN ('Approved', 'Rejected')", con)
            cmd.ExecuteNonQuery()

            ' Delete the rows from table_osrequest after moving them to history
            cmd = New MySqlCommand("DELETE FROM table_osrequest WHERE status2 IN ('Approved', 'Rejected')", con)
            cmd.ExecuteNonQuery()
    ...
    Private Sub txt_search_TextChanged(sender As Object, e As EventArgs) Handles txt_search.TextChanged
        ...
        Try
            con.Open()
            Dim cmd As New MySqlCommand("SELECT requestorname, department, branch, date
                                         FROM  table_osrequest
                                         WHERE (requestorname Like @SearchText Or date Like @SearchText) " &
                                         "And department = 'LRC'", con)
    ...
    Private Sub viewreqstat_Click(sender As Object, e As EventArgs) Handles viewreqstat.Click
        If DataGridView1.Rows.GetRowCount(DataGridViewElementStates.Selected) > 0 Then
            ...
            Dim reqTracClickedForm As New HeadApprover1_ReqTracClicked_Approver()
    ...
    Private Sub LoadFirstName()
        Using connection As New MySqlConnection(str)
            ...
            Dim query As String = "SELECT firstname FROM accountcredentials WHERE usertype = 'HeadApproval1'"
    ...
    Private Sub firstnamedisplay()
        Using connection As New MySqlConnection(str)
            ...
            Dim query1 As String = "SELECT Firstname FROM accountcredentials WHERE usertype = 'HeadApproval1'"
